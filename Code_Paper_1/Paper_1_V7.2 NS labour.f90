...
!
!##############################################################################
include "Paper_1m_V7.2 NS labour.f90"
...
print*, 'wf', wf
        ...
        do ir = 1, NR
            rk(ir) = TProd(ir)*alpha*(KK/LL)**(alpha-1d0)
...
print*, 'pension', pen(JJ, 1)
        ! endogenous lower and upper bound of cash-on-hand grid
        X_l = min(wn(1)*minval(eff(1:JR-1)), pen(JR, 1))
        X_u = (1d0 + rk(NR))*a_u + wn(NR)*maxval(eff(1:JR-1))
        ...
        do ia = 0, NA
            ...
            ! agent is working
                ...
                do ir = 1, NR
                    ...
                    X_p = R_port*a(ia) + wn(ir)*eff(ij+1)
...
do ij= 1, JJ
    ...
    print*, 'ij:', ij
    print*,'@@@@@@@@@@@@@@@@@@@@@@@@'
    print*, 'phi_X:',  phi_X(ij, :)
    print*,'@@@@@@@@@@@@@@@@@@@@@@@@'
    print*, 'a(0:50):', a
    print*,'@@@@@@@@@@@@@@@@@@@@@@@@'
    print*, 'ij:', ij
    print*,'@@@@@@@@@@@@@@@@@@@@@@@@'
    print*, 'phi_a:', phi_a(ij, :)
enddo
...
!~             enddo
    ...
    subroutine aggregation()
        ...
        real*8 :: sigma_eta(JJ), sigma_exp(JJ), mu_avg
        ...
        if(analytical)then
            ...
            do ij = 1, JJ
                ...
                do ir = 1, NR
                    y_coh(ij) = y_coh(ij) + (wn(ir)*eff(ij)&
                                + pen(ij, ir))*dist_TProd(ir)
                enddo

                l_coh(ij) = l_coh(ij) + eff(ij)
...
'DIFF:', DIFF, 'DIFF/YY', (DIFF/YY)*100d0
...
!~             DIFF = YY-CC-II
!~         endif
!~ print*, 'AA', AA, 'KK:', KK, 'BB', BB, 'LL:', LL, 'YY:', YY, 'CC:', CC, 'II:', II, &
!~ 'DIFF:', DIFF, 'DIFF/YY', (DIFF/YY)*100d0
...
print*, 'BB_a:', BB_a, 'BB_b:', BB_b, 'BB_c:', BB_c, 'BB', BB
    ...
    subroutine government()
        ...
        do ij = 1, JJ
            do ir = 1, NR
                total_INC = total_INC + (w(ir)*eff(ij))*m(ij)*dist_Tprod(ir)
            enddo
        enddo
...
print*, 'total_pen:', total_pen, 'PP:', PP, 'total_INC:', total_INC, 'wf*LL:', wf*LL
