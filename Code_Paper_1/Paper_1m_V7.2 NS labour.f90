...
module globals
    ...
    real*8, parameter :: beta = 0.998d0**5 !0.96d0**5
    
    ! production parameters
    real*8, parameter :: alpha = 0.36d0 !0.36d0 !#####################
    real*8, parameter :: delta = 1d0-(1d0-0.0823d0)**5  !#####################
    real*8, parameter :: TProd_bar = 1.2d0 ! i.e. tech grow at 4% per year
    real*8 :: Tprod(1:NR)

    ! risk processes
    real*8, parameter :: sigma_vtheta = 0.157d0**2d0*sqrt(5d0)*0.5d0

    ! Initial risk premium
    real*8, parameter :: mu_r = 0.04d0*5d0
    ...
    ! discretized shocks
    real*8 :: dist_TProd(NR), dist_vtheta(NR)
    real*8 :: vtheta(NR)
    ...
    function foc_port(p)
        ...
        if(ij_com+1 >= JR)then
            ...
            do ir = 1, NR
                ...
                earnings  = wn(ir)*eff(ij_com+1)

                ! get tomorrow's cash on hand
                X_p = R_port*a(ia_com) + earnings
